...
const CompanyInquiries = () => {
    ...
    const fetchInquiries = async () => {
        ...
        try {
            ...
            // Using the unified endpoint for agents and company admins
                            ...
                            <table className="w-full text-left text-sm">
                                <thead className="bg-gray-50 border-b border-gray-100">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 font-semibold text-gray-700 uppercase text-xs">Agent</th>
                                        <th className="px-6 py-4 font-semibold text-gray-700 uppercase text-xs">Property</th>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {inquiries.map((inquiry) => (
                                        <tr key={inquiry.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                {inquiry.property?.user ? (
                                                    <div className="flex items-center gap-3">
                                                        <div className="h-10 w-10 rounded-full bg-orange-50 flex items-center justify-center text-orange-600 font-semibold">
                                                            {inquiry.property.user.firstName?.charAt(0) || 'A'}
                                                        </div>
                                                        <div>
                                                            <div className="font-medium text-gray-900">
                                                                {inquiry.property.user.firstName} {inquiry.property.user.lastName}
                                                            </div>
                                                            {inquiry.property.user.phone && (
                                                                <div className="text-xs text-gray-500 flex items-center gap-1">
                                                                    <Phone className="w-3 h-3" /> {inquiry.property.user.phone}
                                                                </div>
                                                            )}
                                                            {inquiry.property.user.email && (
                                                                <div className="text-xs text-gray-500 flex items-center gap-1">
                                                                    <Mail className="w-3 h-3" /> {inquiry.property.user.email}
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
                                                ) : (
                                                    <span className="text-gray-400 text-sm italic">Company Property</span>
                                                )}
                                            </td>
                                            <td className="px-6 py-4">
