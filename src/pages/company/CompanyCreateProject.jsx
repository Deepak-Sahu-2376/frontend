...
import { X, FileText, Image as ImageIcon, Video, Star } from 'lucide-react';
...
const CompanyCreateProject = () => {
    ...
    // Auto-set company ID from logged-in user
    useEffect(() => {
        if (user) {
            // The company ID might be in user.companyId or user.id depending on user type
            const companyId = user.companyId;
            if (companyId) {
                setFormData(prev => ({ ...prev, companyId: companyId }));
            } else {
                console.warn('No company ID found in user object. Please Try Re-login.');
                toast.error("Company ID missing. Please logout and login again.");
            }
        }
    }, [user]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (parseInt(formData.totalUnits) < 1) {
            ...
            // RERA Validation
            if (formData.reraApproved && !formData.reraValidityDate) {
                toast.error('RERA Validity Date is required when RERA Approved is checked');
                setIsSubmitting(false);
                return;
            }
            ...
            // Prepare projectDTO as JSON string 
            const projectDTO = {
                name: formData.name,
            ...
            if (response.ok) {
                ...
                try {
                    ...
                    toast.error(errorJson.message || 'Failed to create project');
                } catch (e) {
                    toast.error('Failed to create project: Server Error');
                }
            }
        } catch (error) {
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea
                                ...
                                placeholder="e.g., Near Metro Station, Close to Airport"
                    ...
                    <CardContent className="space-y-6">
                        <div className="flex items-center space-x-2">
                            <Checkbox
                                ...
                                onCheckedChange={(checked) => setFormData(prev => ({ ...prev, reraApproved: checked }))}
                            />
