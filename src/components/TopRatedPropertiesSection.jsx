  ...
  useEffect(() => {
    const fetchTopRatedProperties = async () => {
      try {
        ...
        try {
          const response = await fetch(`${API_BASE_URL}/api/v1/public/properties/featured/top?limit=5`);
          if (response.ok) {
            const data = await response.json();
            fetchedProps = Array.isArray(data) ? data : (data.content || []);
          }
        } catch (e) {
          console.warn("Top rated endpoint failed, trying fallback...", e);
        }

        // Fallback to general properties if empty
        if (!fetchedProps || fetchedProps.length === 0) {
          // Fetch top rated (or just listed properties sorted by rating/views if available, else standard list)
          // Adjust endpoint to public one
          const response = await fetch(`${API_BASE_URL}/api/v1/public/properties?page=0&size=6&sort=createdAt,desc`);
          if (response.ok) {
            const data = await response.json();
            fetchedProps = Array.isArray(data) ? data : (data.content || []);
          }
        ...
        if (fetchedProps && fetchedProps.length > 0) {
          const mappedProps = fetchedProps.map(p => ({
            ...
            beds: p.bedrooms || p.bhk || 0,
            baths: p.bathrooms || 0,
            sqft: p.formattedArea || p.carpetArea || p.area || "N/A",
          }));
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 text-lg mt-2">
            Discover our highest-rated properties, loved by buyers and recognized by industry experts.
          </p>
        </div>
            ...
            >
              {extendedProperties.map((p, index) => (
                ...
                >
                  ...
                  <div className="p-5">
                    <div className="text-2xl font-bold text-[#a87a4c] mb-1">{p.price}</div>
                    <div className="text-lg font-semibold line-clamp-1">{p.title}</div>
                    <p className="text-sm text-gray-500 flex items-center gap-1 line-clamp-1">📍 {p.location}</p>
                    <p className="text-sm text-[#c69059] mt-2 flex items-center gap-1 line-clamp-1">✨ {p.highlight}</p>

                    {/* Rating */}
                    <div className="flex items-center gap-2 mt-3">
                      ⭐ {p.rating} <span className="text-gray-500 text-sm">({p.reviews})</span>
                    </div>
...
export default TopRatedPropertiesSection;
